...
if [ ${typeEnv} == "preprod" ]; then
    serverDir="preprod"
    gitBranch="release"
    message="Testez l'url http://site.preprod.g2-lpdiwa.edu/. Il faut attendre plusieurs minutes. Si cela ne fonctionne toujours pas analysez les logs des conteneurs."

else
  if [ ${typeEnv} == "prod" ]; then
    serverDir="prod"
    gitBranch="master"
    message="Testez l'url http://la-diwa-02.univ-lemans.fr/. Il faut attendre plusieurs minutes. Si cela ne fonctionne toujours pas analysez les logs des conteneurs."
  fi
fi
...
if [ -d ./2122-LPDIWA-PROJET-G2 ]; then
  cd ./2122-LPDIWA-PROJET-G2/script
  ./dcTool prod -lmp down -v
  cd ../..
  sudo rm -rf ./2122-LPDIWA-PROJET-G2/
fi

#git clone -b ${gitBranch} https://github.com/IUT-LAVAL-MMI/2021-LPDIWA-PROJET-G2.git
git clone -b demo git@github.com:/IUT-LAVAL-MMI/2122-LPDIWA-PROJET-G2.git
cd ./2122-LPDIWA-PROJET-G2
sudo rm -rf ./.git
